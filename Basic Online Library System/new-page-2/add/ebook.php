...
<div class="art-box art-vmenublock">
	...
	<li>
		...
		<ul class="active">
			...
			<li>
                <a href="../../new-page-2/add/ebook.php" class="active">Ebook</a>
...
$checkSql="select count(id) as eCount from ebook";
...
$sql = "INSERT INTO ebook (url, name) VALUES ('" . $url . "', '" . $name . "')";
...
<input type="text" value="<?=$_GET['url'];?>" name="url" />
</div></td></tr>
...
$sql = "UPDATE ebook SET url = '" . $url ."', name = '" . $name . "' WHERE id = '" . $id . "'";
mysql_query($sql);
...
$sql ="SELECT * FROM ebook ORDER BY name ASC";
...
?>
<td><a href="&url=<?=$info['url'];?> "> click me </a>  </td>
<td><?=$info['name'];?></a></td>
<td><a href="<?=$_SERVER['PHP_SELF'];?>?id=<?=$info['id'];?>&url=<?=$info['url'];?>&name=<?=$info['name'];?>&mode=edit" >Edit </a></td>
