...
$title2 = "We are editing: " . $_GET['title'] . " are you sure!!!";
}
?>
...
<div class="art-box art-vmenublock">
	...
	<li>
		...
		<ul class="active">
			<li>
                <a href="../../new-page-2/add/book.php" class="active">Book</a>

            </li>
			<li>
                <a href="../../new-page-2/add/magazine.php">Magazine</a>
...
<tr><td>Author:</td><td><div align="left">
...
$sql = "INSERT INTO lib (title, author) VALUES ('" . $title . "', '" . $author . "')";
...
<input type="text" value="<?=$_GET['title'];?>" name="title" />
</div></td></tr>

<tr><td>Author:</td><td><div align="left">
<input type="text" value="<?=$_GET['author'];?>" name="author" />
</div></td></tr>
...
$sql = "UPDATE lib SET status='Available', title = '" . $title ."', author = '" . $author . "' WHERE id = '" . $id . "'";
mysql_query($sql);
...
$sql ="SELECT * FROM lib ORDER BY title ASC";
$data = mysql_query($sql);

?>
<h2>Add, Update and remove Books</h2>
<table class="tableStyleClass">
<tr>
<th width="100">Title of the book</th>
<th width="200">Name of the Author</th>
<th width="100" colspan="2">Admin</th>
</tr>
<td colspan="5" align="right"><?php if($disable!=1){?><div align="right"><a href="<?=$_SERVER['PHP_SELF'];?>?mode=add"?mode=add>Add a Book</a><?php }else{?>Contact Book is Full<?php } ?></div></td>
<?php
...
<td><?=$info['title'];?></td>
<td><?=$info['author'];?></a></td>
<td><a href="<?=$_SERVER['PHP_SELF'];?>?id=<?=$info['id'];?>&title=<?=$info['title'];?>&author=<?=$info['author'];?>&mode=edit" >Edit </a></td>
